package com.bank.manage.application.views;
...
//            }
    ...
    private <T> void addLabel(T data, int position, LabelTextProvider<T> provider) {
        ...
        label.setLayoutParams(new LayoutParams(-2, LayoutParams.WRAP_CONTENT));
        ...
        if(drawableRight != 0){
            label.setCompoundDrawablesWithIntrinsicBounds(0,0, drawableRight, 0);
            label.setCompoundDrawablePadding((int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP,6,getResources().getDisplayMetrics()));
        }
        if(mLabelBg != null){
