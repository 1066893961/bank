...
public class HttpHelper {
    ...
    public static void getFeeList(Object tag, JsonCallback callback) {
        ...
        OkGo.get(AddressContants.API_SERVER_FEE_LIST + "?page=1&size=100")
                .tag(tag)
                .execute(callback);
    }
    ...
    public static void getCommonList(Object tag, int type, JsonCallback callback) {
        String url = "";
        if (type == 2) {
            url = AddressContants.API_SERVER_BANK_LIST + "?page=1&size=100&userId=" + Preferences.getUserId();
        } else {
            url = AddressContants.API_SERVER_ZHAUNZHANG_LIST + "?page=1&size=100&userId=" + Preferences.getUserId();
        }
     ...
     * @param callback
     */
    public static void modifyPsd(Object tag,  String oldPsd, String psd, String payPsd, JsonCallback callback) {
        OkGo.post(AddressContants.API_SERVER_MODIFY_PSD)
                .tag(tag)
                .isMultipart(true)
                .headers("Content-Type", "multipart/form-data; boundary=;")
                .params("id", Preferences.getUserId())
                .params("oldPassword", oldPsd)
                .params("password", psd)
                .params("payPassword", payPsd)
                .execute(callback);
    }
